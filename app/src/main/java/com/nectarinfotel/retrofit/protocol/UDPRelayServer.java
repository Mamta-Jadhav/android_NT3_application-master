...
class UDPRelayServer implements Runnable {
	...
	public void start() throws IOException {
		...
		Log.d("test","Starting UDP relay server on {}:{}"+ relayIP+" "+ relayPort);
		Log.d("test","Remote socket {}:{}" + remote_sock.getLocalAddress()
	...
	public void run() {
		try {
			...
			abort();
			Log.d("test","UDP Pipe thread " + Thread.currentThread().getName()
					+ " stopped.");
		}
	...
	private synchronized void abort() {
		...
		Log.d("test","Aborting UDP Relay Server");
	...
	private void pipe(DatagramSocket from, DatagramSocket to, boolean out)
		...
		while (true) {
			try {
				...
				Log.d("test","Dropping datagram for unknown host");
