...
public class Socks5DatagramSocket extends DatagramSocket {
	...
	public Socks5DatagramSocket(SocksProxyBase p, int port, InetAddress ip)
		...
		Log.d("test", "Datagram Socket:{"+getLocalPort()+"}:{"+getLocalAddress()+"}");
		Log.d("test", "Socks5Datagram: {"+relayIP+"}:{"+relayPort+"}");
	}
	...
	public void send(DatagramPacket dp) throws IOException {
		...
		if (!server_mode && proxy.isDirect(dp.getAddress())) {
			super.send(dp);
			Log.d("test","Sending datagram packet directly:");
			return;
		}
		...
		if (proxy != null) {
			try {
				...
				if (eof < 0) {
					...
					Log.d("test", "This really should not happen");
